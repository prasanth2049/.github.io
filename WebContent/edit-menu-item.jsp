...
<body>
	<header> <span id="head">truYum</span> <img class="img"
	...
	<div>
		...
		<form class="cont" name="form" onSubmit="return validation()"
			method="post" action="EditMenuItem?id=${menuItem.id }">
			<table>
				<tr>
				...
				<tr>
					<td><input type="text" class="title-price" id="txtPrice"
						name="txtPrice" value="${menuItem.price}"></td>

					<td><label for="rdYes"><input type="radio" id="rdYes"
							name="rdYes" value="Yes"
							<c:if test="${ menuItem.active eq 'true'}"> checked="checked"</c:if>>Yes</label>

						<label for="rdNo"><input type="radio" id="rdNo"
							name="rdYes" value="No"
							<c:if test="${ menuItem.active eq 'false'}"> checked="checked"</c:if>>No</label></td>

					<td><fmt:formatDate type="date" pattern="dd/MM/yyyy"
							value="${menuItem.dateOfLaunch}" var="format" /><input
						type="text" id="txtDoB" name="txtDoB" value="${format }"></td>
